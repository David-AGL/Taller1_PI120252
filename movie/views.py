...
def home(request):
    ...
    #return render(request, 'home.html',{'name':'David Gutiérrez'})
...
def statistics_by_genre_view(request):
    matplotlib.use('Agg')

    # Obtener todas las películas
    all_movies = Movie.objects.all()

    # Diccionario: cantidad de películas por género (solo el primero)
    movie_counts_by_genre = {}

    for movie in all_movies:
        # Tomar solo el primer género desde el CharField 'genre'
        raw = (movie.genre or "").strip()
        first_genre = raw.split(',')[0].strip() if raw else "None"
    ...
    return render(request, 'statistics_genres.html', {'graphic': graphic})
